...
def _add(by_cat, seen, host, cat, src, url, price, has_schema, desc):
    key = (host, cat)
    if key in seen:
        return
    seen.add(key)
    ...
    by_cat[cat]["sellers"].append({
        "host": host, "url": url, "price_usdc": price,
        "has_schema": bool(has_schema), "description": (desc or "")[:80]})


def build_map():
    """Classify the WHOLE market = CDP registry UNION x402scan sellers.

    Was cdp_resources_raw.json ALONE -- the Coinbase directory, not the market --
    which silently dropped every x402scan-only seller (~35% of seller dollars, incl.
    the #2 seller api.clusterprotocol.ai) from categorization, accuracy grading, and
    find_api. This merges two sources so we lose nothing: (1) the full CDP resource
    list per host (complete per-endpoint coverage the accuracy categories need), then
    (2) the union hosts probe found (latest.json), which fills in every x402scan-only
    seller CDP never listed. Dedup is by (host, category), CDP first, so CDP's
    complete endpoint coverage is preserved and the x402scan half is added on top."""
    by_cat, seen = {}, set()

    # 1. CDP registry: full resource list per host (complete endpoint coverage).
    for r in json.load(open(os.path.join(DATA, "cdp_resources_raw.json"))):
        url = r.get("resource") or ""
        host = (urlparse(url).hostname or "").lower()
        if not (host and url.startswith("http")):
            continue
        cat, src = categorize(url, r.get("description") or "", r.get("tags") or "")
        _add(by_cat, seen, host, cat, src, url, conform.price_of(r),
             conform.contract(r), r.get("description"))

    # 2. The union probe actually graded (CDP + x402scan). Adds x402scan-only sellers
    #    CDP never listed; (host, category) dedup means it only ever fills gaps.
    for o in json.load(open(os.path.join(DATA, "latest.json"))).get("origins", []):
        host = (o.get("origin") or "").lower()
        if not host:
            continue
        tags = o.get("tags") or []
        blob = " ".join(str(x) for x in (
            o.get("service"), o.get("blurb"),
            " ".join(o.get("descriptions") or []), " ".join(str(t) for t in tags)) if x)
        eps = [c for c in (o.get("checked") or [])
               if isinstance(c.get("url"), str) and c["url"].startswith("http")]
        if not eps:                                 # probe could not reach it (common
            eps = [{"url": f"https://{host}"}]       # for x402scan-only): host-level, never invisible
        for c in eps:
            cat, src = categorize(c["url"], blob, " ".join(str(t) for t in tags))
            _add(by_cat, seen, host, cat, src, c["url"],
                 c.get("adv_amount") or c.get("live_amount"), c.get("schema"), blob)
    return by_cat
