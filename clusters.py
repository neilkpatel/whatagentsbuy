        ...
        for r in rows:
            ...
            if h:
                ...
                demand[h] = {"label": r.get("demand"), "ods": r.get("organic_score"),
                             ...
                             "top_payer_share": r.get("top_payer_share")}
...
def build():
    ...
    for t, hosts in seen.items():
        ...
        # "real" demand: strong organic score AND not dominated by a single wallet
        # (concentration is the honest wash proxy; the retired payout flag was not).
        real = [d for d in dd if (d["ods"] or 0) >= 60 and (d.get("top_payer_share") or 0) < 0.9]
...
def main():
    ...
    print("real    = of those, how many score 60+ on Organic Demand with no single wallet above 90%")
