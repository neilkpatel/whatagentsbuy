...
def contract(r):
    ...
    if method in ("POST", "PUT", "PATCH") and not isinstance(body, dict):
        return None
    q = inp.get("queryParams")
    return {"method": method, "body": body, "expect": sorted(ex.keys()),
            "expect_example": ex, "query": q if isinstance(q, dict) and q else None}


def with_query(url, query):
    """Append a seller's declared query params to a URL. Many GET endpoints 404
    or 400 without them (geoprimitives wants lat/lon), so calling the bare URL
    read as 'dead' when the seller had told us exactly what to pass. Respects an
    existing query string."""
    if not query:
        return url
    sep = "&" if urlparse(url).query else "?"
    return url + sep + urlencode(query)
...
def archive_raw(url, method, body, returncode, stdout, stderr, out_dir=None, call_id=None):
...
def call(url, method, body, cap, timeout=45, query=None):
    ...
    # Immutable call identity, minted BEFORE dispatch and carried through the
    # archive line, the returned meta, and (via the caller) the conformance row.
    # Receipts used to re-guess which archived capture backed a row by URL+tx,
    # and 186 published receipts lost their binding to query-param drift; an id
    # created at dispatch cannot drift.
    call_id = "c_" + uuid.uuid4().hex[:16]
    ...
    try:
        ...
        archive_raw(url, method, body, None, "", "client timeout", call_id=call_id)
        return False, None, {"error": "client timeout", "_call_id": call_id}
    # Save the untouched response FIRST, before a single parse. Everything below
    # is derived and re-derivable from this line.
    archive_raw(url, method, body, p.returncode, p.stdout or "", p.stderr or "", call_id=call_id)
    raw = (p.stdout or "").strip()
    if not raw:
        return False, None, {"error": (p.stderr or "no output")[:200], "_call_id": call_id}
    try:
        env = json.loads(raw)
    except ValueError:
        return False, None, {"error": raw[:200], "_call_id": call_id}
    if not isinstance(env, dict):
        return False, None, {"error": "unexpected envelope", "_call_id": call_id}
    ...
    if env.get("success"):
        ...
        meta = {"payment": m.get("payment"), "price": m.get("price"),
                "status": m.get("statusCode"), "_call_id": call_id}
...
def describe_shape(value, depth=0):
    ...
    if isinstance(value, dict):
        if depth >= 3:
            return "object"
        # No key cap. A [:40] truncation here once made a wide response's
        # promised fields VANISH from the saved shape, so an offline regrade
        # could turn a delivered call into a short. Keys are only names and
        # type leaves; depth (<=3) already bounds the size.
...
def run_canary(cap=0.01):
    ...
    Returns (ok, detail, spent). Canary calls are PAID calls, so what they cost
    is returned for the caller to count against the run budget: they used to
    spend outside the ledger entirely, so a run could pass its budget before
    the first graded call."""
    spent = 0.0
    for c in CANARY:
        ok, payload, meta = call(c["url"], c["method"], c["body"], cap, query=c.get("query"))
        ...
        if paid.get("success"):
            ...
            if isinstance(meta, dict) and meta.get("price"):
                try:
                    ...
                    charged = None
            spent += min(charged, cap) if charged is not None else cap
        v = judge(payload, c["expect"])
        if v["status"] != "delivered":
            return False, (f"canary {c['url']} scored '{v['status']}' not 'delivered'. "
                           f"The harness is broken, not the endpoint. why: {v['why']}"), spent
    return True, f"{len(CANARY)} known-good endpoints all scored delivered", spent
...
def main():
    ...
    ap.add_argument("--max-price", type=float, default=0.10,
                    help="per-call price ceiling; the run --budget bounds total spend, not this")
    ...
    # Canaries are paid calls: their maximum cost is reserved against the budget
    # BEFORE they run and their actual cost counts against it after.
    canary_spent = 0.0
    if not a.dry_run:
        _canary_reserve = len(CANARY) * 0.01
        if _canary_reserve > a.budget:
            print(f"refusing to start: the canary alone can cost ${_canary_reserve:.2f}, "
                  f"over the ${a.budget:.2f} budget.")
            return 1
        ok_c, detail_c, canary_spent = run_canary()
        if not ok_c:
            print(f"\nABORTING: {detail_c}\nFix the harness and re-run. No verdicts recorded.")
            return 1
        print(f"canary passed: {detail_c} (cost ${canary_spent:.4f}, counted against the budget)\n")
    ...
    for i, c in enumerate(picked, 1):
        # Per-call ceiling: this endpoint's own advertised price plus a hair for
        # rounding, capped at --max-price. The RUN BUDGET is the real safety bound
        # on total spend; a hardcoded 1-cent backstop used to sit here too, but it
        # silently excluded every endpoint over 1c (LLM, image, web search, real
        # goods) from delivery grading regardless of --max-price -- a coverage
        # ceiling masquerading as a money guard. If the live 402 asks for more than
        # the cap, the client aborts and pays nothing, recorded as a price mismatch.
        per_call_cap = min(round(c["price"] * 1.25 + 0.0001, 6), a.max_price)
        # Reserve the MAXIMUM this attempt is authorized to debit (the cap), not
        # the advertised price: caps run 25% over quote, so budgeting on quotes
        # let a full run overshoot its budget by up to a quarter.
        if spent + per_call_cap > a.budget:
            print(f"\nstopping at {i-1}: reserving the next call's ${per_call_cap:.4f} cap "
                  f"would pass the budget")
            break
        t0 = time.time()
        ok, payload, meta = call(c["url"], c["method"], c["body"], per_call_cap, query=c.get("query"))
        ...
        if v.get("status") == "short":
            if spent + per_call_cap > a.budget:
                # The confirmation call does not fit the budget, and guardrail 3
                # forbids recording an accusation confirmed only once. The verdict
                # downgrades rather than the budget stretching or the retry being
                # silently skipped: the old unconditional retry could overspend.
                v = {"status": "inconclusive", "conforms": False, "missing": c["expect"],
                     "extra": [], "why": "short on first call; re-verification did not fit "
                                         "the run budget, and an unconfirmed negative is never recorded",
                     "observed_schema": v.get("observed_schema")}
            else:
                ok2, payload2, meta2 = call(c["url"], c["method"], c["body"], per_call_cap, query=c.get("query"))
    ...
    # Reconcile EVERY settled charge against the chain BEFORE promoting, in both
    # directions. agentcash's meta.price over-reported ($0.02 on $0.015 sellers
    # who, on chain, received exactly $0.015), and an under-report would hide a
    # real overcharge just as silently. The chain is truth; rows whose RPC lookup
    # fails stay explicitly charged_verified=false rather than reading as fact.
    try:
        import reconcile_charged
        _ck, _fx, _cl, _uv = reconcile_charged.reconcile(out_path)
        if _ck or _uv:
            print(f"  reconciled {_ck} settled charges against the chain "
                  f"({_fx} corrected, {_cl} phantom overcharges cleared, {_uv} unverified)")
    except Exception as _e:
        print(f"  (charge reconciliation skipped: {_e})")

    # Promote these clean grades into conformance_verified.json right now, so the
    # verified/preflight tier and the MCP reflect them the same day. Without this
    # the dated grades never counted and coverage silently froze at 350 for two
    # weeks. Best-effort: a promotion failure never fails the (already-paid) run.
    try:
        import promote_verified
        _rows = json.load(open(out_path)).get("rows", [])
        _added, _updated, _tot = promote_verified.promote_verified(_rows)
        print(f"  promoted to verified: +{_added} new, {_updated} refreshed ({_tot} verified rows)")
    except Exception as _e:
        print(f"  WARNING: promotion to verified failed ({_e}); run: python3 promote_verified.py")
