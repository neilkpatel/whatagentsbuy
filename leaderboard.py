...
def organic_score(payers, repeat, top1, hhi, settlements):
    ...
    cost of goods as it is recycling. The payout ratio is kept as a raw chain
    fact only; it is never turned into a wash verdict, because it cannot tell a
    recycler from an operational treasury or a reseller. Demand-realness rests on
    payer concentration and this score alone.
...
def main():
    ...
    for label, n in (("1d", 1), ("7d", 7), ("30d", 30)):
        ...
        for addr, v in agg.items():
            ...
            rows.append({
                ...
                # Outflow is kept as a raw chain fact only, never turned into a
                # wash verdict. The old `circular` flag (usdc_out/usdc >= 0.9)
                # conflated three different things: a recycler, an operational
                # treasury with unrelated outflows (blockrun sent out ~34x its
                # x402 income), and a broad reseller passing through cost of goods.
                # Demand-realness is judged by payer concentration and ODS instead,
                # the same basis market.py uses. Retired 2026-08-30.
    ...
    for i, r in enumerate(w["rows"][:12], 1):
        ...
        print(f"{i:2d} {r['service'][:30]:30s} ${r['usdc']:>10,.2f} {pct:>8}")
    for label in ("7d", "30d"):
